...
def C_D_n_cmax_batchwise(window_duration):
	...
	#Calculating the entire C before and after the gw
	C_before=np.empty(2*tau_max_index+1)
	C_after=np.empty(2*tau_max_index+1)

	for i in np.arange(-tau_max_index, tau_max_index+1):
		C_before[i+tau_max_index]=stats.pearsonr(strain_H1_filt[index_before_start+i:index_before_end+i], strain_L1_filt[index_before_start:index_before_end])[0]

	for i in np.arange(-tau_max_index, tau_max_index+1):
		C_after[i+tau_max_index]=stats.pearsonr(strain_H1_filt[index_after_start+i:index_after_end+i], strain_L1_filt[index_after_start:index_after_end])[0]

	C_before-=np.mean(C_before)
	C_before/=np.std(C_before)
	C_after-=np.mean(C_after)
	C_after/=np.std(C_after)    #Normalize C to mean 0, std 1
	...
	plt.figure()
	plt.plot(1000*tau_range, C_before, 'b')
	plt.plot(1000*tau_range, D_before, 'r')
	plt.xlabel(r'$\tau\ (ms)$')
	plt.ylim([-2, 2.5])
	plt.legend([r'$C_{before\ gw}(\tau)$', r'$D_{before\ gw}(\tau)$'], loc='upper left')
	plt.savefig('CD_before_w_point'+str(int(10*window_duration))+'.png')

	plt.figure()
	plt.plot(1000*tau_range, C_after, 'b')
	plt.plot(1000*tau_range, D_after, 'r')
	plt.xlabel(r'$\tau\ (ms)$')
	plt.legend([r'$C_{after\ gw}(\tau)$', r'$D_{after\ gw}(\tau)$'], loc='upper left')
	plt.ylim([-2, 2.5])
	plt.savefig('CD_after_w_point'+str(int(10*window_duration))+'.png')
	...
	plt.savefig('C_max_vs_batch_time_w_point'+str(int(10*window_duration))+'.png')
